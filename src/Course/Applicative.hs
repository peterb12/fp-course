...
instance Applicative List where
  ...
  (<*>) ::
    ...
    -> List a
    -> List b
  (<*>) fs as = foldRight (\f acc -> (map f as) ++ acc) Nil fs
...
(<*) ::
  ...
  -> k a
  -> k b
(<*) kb ka = lift2 (flip (const id)) kb ka
...
replicateA ::
  ...
  -> k a
  -> k (List a)
--replicateA n ka = lift1 (replicate n) ka  -- nope
...
filtering ::
  ...
  -> List a
  -> k (List a)
filtering p = foldRight (\x acc -> lift2 (f x) (p x) acc) (pure Nil)
